...
#+begin_src emacs-lisp
  (defun dan/org-babel-switch-to-code-with-session (&optional arg info)
...
#+begin_src emacs-lisp
  ...
  (defun dan/lgrep-in-org-src (beg end)
    (interactive "r")
    (save-restriction
      (narrow-to-region beg end)
      (lgrep (buffer-string) "*.el" (concat dan/org-mode-src-dir "/lisp"))
      (widen)))
#+end_src
...
# #+end_src
#
#+begin_src emacs-lisp :tangle no :eval no :expand yes :results silent :var R-builtins=R-builtins()
  (add-to-list
...
#+begin_src emacs-lisp
  ...
  (setq dan/gnus-summary-line-format
        (concat
         "%0{%U%R%z%}"
         "%3{│%}" "%1{%d%}" "%3{│%}" ;; date
         "  "
  ...
  (setq dan/gnus-summary-line-format:henri-paul-indiogine
        (concat
         "%0{%U%R%z%}"
  ...
  (setq gnus-summary-line-format dan/gnus-summary-line-format:henri-paul-indiogine)
