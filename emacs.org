...
(defun counsyl/python-fix-up-fill-paragraph-in-docstring ()
  "Fix up formatting of python docstrings after fill-paragraph.

  Place the first line on its own line, and don't insert a blank
  line at the end."
  (interactive)
  (save-excursion
    (re-search-backward "\"\"\"" nil t)
...
(defadvice python-fill-paragraph (after fix-up-fill-paragraph-in-docstring)
  (counsyl/python-fix-up-fill-paragraph-in-docstring))

(ad-deactivate 'python-fill-paragraph)
#+end_src
