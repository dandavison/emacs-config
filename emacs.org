...
(defvar dan/mode-line-force-color-update-hack t)
...
(defun dan/highlight ()
  ...
  (if dan/highlighted
    ...
    (let ((tap (thing-at-point 'symbol)))
       (highlight-regexp tap 'isearch-fail)
       (setq dan/highlighted tap))))
...
;; Should move to something based on this:
(defun dan/set-exec-path-from-shell-PATH ()
  (let ((path-from-shell 
         (replace-regexp-in-string
          "[[:space:]\n]*$" "" 
...
(defun dan/ipython (&optional restart)
  (interactive "P")
  (dan/python dan/ipython-command restart))

(defun dan/ipython-console (&optional restart)
  (interactive "P")
  (dan/python
...
(defun dan/dict-literal-to-kwargs ()
  (interactive)
  (save-excursion
...
(defun dan/kwargs-to-dict-literal ()
  (interactive)
  (save-excursion
    (goto-char (region-beginning))
    (while (re-search-forward "'\([^ ]+\)'=\([^ ]+\)," (region-end) t)
...
(add-hook 'python-mode-hook 'paredit-not-lisp-mode)
...
#+begin_src emacs-lisp
(require 'paredit)
(add-to-list 'load-path "~/src/emacs/paredit-not-lisp")
(require 'paredit-not-lisp)
...
;; (add-hook 'ess-mode-hook 'paredit-not-lisp-mode)
...
(add-hook 'inferior-ess-mode-hook 'paredit-not-lisp-mode)
(add-hook 'comint-mode-hook 'paredit-not-lisp-mode)
