...
(defun dan/codequality (&optional dir)
  ...
  (compile
   (format
    "find %s -type f -name '*.py' | xargs codequality '{}' \;" dir)))
#+end_src
...
(defun dan/dump-comint-history (&optional file)
  ...
  (let ((history
         (mapconcat #'identity
                    (org-uniquify (ring-elements comint-input-ring))
                    "\n")))
    (with-temp-buffer
    (insert history "\n")
...
(add-hook 'inferior-python-mode-hook
          (lambda () (dan/load-comint-history "~/.ipython/history")))
...
(defun dan/conditionally-enable-paredit-mode ()
  "enable paredit-mode during eval-expression"
  (if (eq this-command 'eval-expression)
      (paredit-mode 1)))
(add-hook 'minibuffer-setup-hook 'dan/conditionally-enable-paredit-mode)
...
(defun dan/org-table-import ()
  ...
  (let* ((table-file buffer-file-name)
    ...
    (if (get-buffer org-buffer)
        (error "Buffer already exists: %s" org-buffer)
...
(dan/register-key-bindings
 '("inferior-python" .
   (([C-tab] . python-shell-completion-complete-or-indent))))
#+end_src
*** python
#+begin_src emacs-lisp :tangle yes
(dan/register-key-bindings
...
** Face hacks
#+begin_src emacs-lisp :tangle no
(set-face-attribute 'py-pseudo-keyword-face nil :foreground "salmon")
(set-face-attribute 'py-builtins-face nil :foreground "salmon")
