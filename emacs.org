...
[[http://thread.gmane.org/gmane.emacs.orgmode/8112]]
#+begin_src emacs-lisp
  (load "/usr/local/src/emacs/nxhtml/autostart.el")
  (require 'mumamo)
    ...
    (mapcar
     (lambda (mode)
       (eval `(defun ,(intern (format "mumamo-chunk-org-%S" mode)) (pos min max)
                ,(format "%s support inside org code blocks" mode)
                (mumamo-quick-static-chunk
                 pos min max ,(format "#+begin_src %S" mode) "#+end_src"
                 t (quote ,mode) nil))))
     mumamo-org-modes)
...
(org-fontify-meta-lines-and-blocks 84237)
#+begin_src emacs-lisp
