...
(defun dan/highlight (&optional arg)
  ...
  (if dan/highlighted
    ...
    (if arg
        (setq dan/highlighted (read-from-minibuffer "Highlight: "))
        (let ((tap (thing-at-point 'symbol)))
...
(defun dan/set-exec-path-from-shell-PATH ()
  (let ((path-from-shell 
         (replace-regexp-in-string
          "[[:space:]\n]*$" "" 
          (shell-command-to-string "zsh -c '. ~/.zshrc 2>&/dev/null && echo $PATH'"))))
    (setenv "PATH" path-from-shell)
    (setq exec-path (split-string path-from-shell path-separator))))
(dan/set-exec-path-from-shell-PATH)
#+end_src
